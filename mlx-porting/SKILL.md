...
- [ ] **Wrapper call chain** — port the code the reference *actually invokes*, not just the network module. Third-party wrappers (`RealESRGANer.enhance()`, diffusers pipeline helpers) own preprocessing, padding, channel conventions, and output quantization (`.round()` vs a truncating `astype`); rewriting them from scratch instead of transposing them line by line is where divergence hides. Read the caller and the callee TOGETHER — contract violations between them are invisible in either file alone (Hunyuan3D fed PIL-RGB into cv2-BGR-assuming `enhance()`, so the reference net ran channel-swapped). Parity tests inject prepared tensors and never cover these edges; when the reference behavior is itself a bug, deviate deliberately and document it, don't reproduce it blindly or silently.
